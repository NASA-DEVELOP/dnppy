...
def Reference_ET(Del, R_n, G_shf, gam, T_C, u2, e_s, e_a):
    ...
    ET_ref = (0.408 * Del * (R_n - G_shf)) + (gam * (900 / (T_C + 273.16)) *
                 u2 * (e_s - e_a)) / (Del + gam * (1 + 0.34 * u2))
    return ET_ref
...
def Wx_Data_Extract_MID_ATLANTIC(fwx, year, month, day, hours, minutes):
...
def Num7(delta, phi, s, gamma, omega):
    cos_theta_rel = ( math.sin(delta) * math.sin(phi) * arcpy.sa.Sin(s) ) - ( math.sin(delta) * math.cos(phi) * arcpy.sa.Sin(s) * arcpy.sa.Cos(gamma) ) + ( math.cos(delta) * math.cos(phi) * arcpy.sa.Cos(s) * math.cos(omega) ) + ( math.cos(delta) * math.sin(phi) * arcpy.sa.Sin(s) * arcpy.sa.Cos(gamma) * math.cos(omega) )  + ( math.cos(delta) * arcpy.sa.Sin(gamma) * arcpy.sa.Sin(s) * math.sin(omega) )
...
def Num15(asr_list, coefficient_list):
    outMath = asr_list[0] * coefficient_list[0]
    count = 1
    for raster in asr_list[1:]:
        outMath += asr_list[count] * coefficient_list[count]
        count += 1
    return outMath
...
def Num40(Num37, Num38, Num20, Num28):
    #k = von Karmen's constant = 0.41
    outMath = - ( Num77 * 1000 * Num38 ** 3 * Num 20 ) / ( 0.41 * 9.807 * Num28 )
    return outMath
...
def Num52(LE, gamma):
    outMath = 3600 * ( LE / ( gamma * 1000 ) )
    return outMath

#Equation 53- Latent Heat of Vaporization
def Num53(sfcTemp):
    outMath = (2.501 - 0.00236 * ( sfcTemp - 273.15 ) ) * 10 ** 6
    return outMath
