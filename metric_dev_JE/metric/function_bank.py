...
'''
def Wx_Data_Extract_Idaho(time):
    IDWeather = "C:\\Users\\nqian\\Downloads\\9321966457550dat.txt" #need to insert relative filepath
...
def Wx_Data_Extract_NC(time):
    IDWeather = "C:\\Users\\nqian\\Downloads\\july2013.txt"
    weatherdata = open(IDWeather, 'r')
    rows = []
    for line in weatherdata:
        rows.append(line.split())
...
def Num28(Num37, Num29, Num30):
    outMath = Num37 * 1005 * ( Num29 / Num30 )
    return outMath
...
def Num35(Num33, s):
    zom = Num33 * ( 1 + ( (180 / math.pi ) * s - 5 ) / 20 )
    return zom
